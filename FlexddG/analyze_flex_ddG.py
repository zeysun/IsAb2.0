...
def final_output(script_output_folder):
    csv_file=os.path.join(script_output_folder, 'output_saturation-results.csv')
    result_data=pd.read_csv(csv_file, low_memory=False)

    filter_data=(result_data['score_function_name']=='fa_talaris2014-gam') & (result_data['nstruct']==35) & (result_data['total_score'] < 0)
    result_filter=result_data[filter_data]
    final_result=result_filter[['case_name', 'total_score']].copy()
    final_result=final_result.reset_index(drop=True)
    final_result.to_csv(r'result.csv', index=False)
...
if __name__ == '__main__':
    ...
    final_output(script_output_folder)
